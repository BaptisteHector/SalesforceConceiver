...
export class ForceService {
  ...
  appId = '3MVG9t0sl2P.pByqE.XiWMYfMKlc6oZ5EFJgGO5_cgs3ra0Hj1bzvLUiVvYYEI9dQ2jeokpMrB36yfwcUJlPB';
  ...
  proxyURL = 'https://immasayyes-dev-ed.my.salesforce.com/';
  ...
  init = (params: any) => {
    ...
    console.log("Force service Init.");
  ...
  loginWithBrowser = () => new Promise<void>((resolve, reject) => {
    if (localStorage.getItem('SFToken')) {
      ...
      this.context = "";
      console.log('Oauth FOrce Service', this.oauth);
      ...
      document.addEventListener('oauthCallback', (evt) => {
        ...
        if (url.indexOf('access_token=') > 0) {
          ...
          console.log('sftoken', localStorage.getItem('SFToken'));
          this.oauth = JSON.parse(localStorage.getItem('SFToken')!);
          console.log('oauth', this.oauth);
          console.log('oauth id', this.oauth.id);
          this.tokenStore.forceOAuth = this.oauth;
          this.loginCalled = false;
          resolve();
  ...
  request = (obj: any) => new Promise((resolve, reject) => {
    ...
    console.log("OAUTH", this.oauth);
    console.log("ACCESS TOKEN", this.oauth.access_token);
